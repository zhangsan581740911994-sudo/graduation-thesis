...
### 任务 B：深度消融实验（后台正在运行，验证 80% 行为噪声）
由于在 20 只股票的“干净数据”任务中，EDP 和纯 TD3 表现接近，为了在学术上证明 EDP（扩散模型）的绝对优势，我们正在后台运行一个**“80% 混合噪声”**的深度消融实验。

- **当前状态**：已通过 `scripts/run_ablation_noise.sh` 在 AutoDL 后台启动。
- **预期**：纯 TD3 在面对这种“多模态、嘈杂”的次优数据时，性能会大幅崩溃；而 EDP 作为生成模型，能完美拟合复杂分布并提取高回报动作，依然保持高收益。
- **战略意义**：如果此实验成功，将作为论文的**核心预研成果**（见 `论文结果_填写版.md` 第 9 节），甚至可能替换前文的主线逻辑。
